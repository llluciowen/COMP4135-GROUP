...
def diversity_rerank(
    ...
    """Light MMR-style rerank: penalises overlapping genres (diversity) and rewards
    newer release years (recency). Both knobs are in [0, 1]; 0 means pure relevance.
    Operates on a list of movie dicts in-place-safe (returns a new list).
    """
    if not candidates:
        return candidates
    diversity = max(0.0, min(1.0, float(diversity)))
    recency = max(0.0, min(1.0, float(recency)))
    if diversity == 0.0 and recency == 0.0:
        return candidates

    # Normalise base scores
    raw_scores = []
    for movie in candidates:
        try:
            raw_scores.append(float(movie.get(score_field) or 0.0))
        except (TypeError, ValueError):
    ...
    # Year normalisation for recency
    years = []
    for movie in candidates:
        y = movie.get('year')
        try:
            ...
            y_num = float('nan')
        years.append(y_num)
    valid_years = [y for y in years if y == y]  # filter NaN
    if valid_years:
        y_min, y_max = min(valid_years), max(valid_years)
        y_span = (y_max - y_min) or 1.0
    else:
        y_min, y_span = 0.0, 1.0
    year_norm = [(y - y_min) / y_span if y == y else 0.5 for y in years]
    ...
    while remaining:
        ...
        for pos, idx in enumerate(remaining):
            base = norm_scores[idx]
            genres = candidates[idx].get('genres') or []
            if isinstance(genres, str):
                genres = genres.split('|')
            overlap = len(set(genres) & selected_genres) if selected_genres else 0
            div_penalty = diversity * (overlap / max(1, len(genres))) if genres else 0.0
            recency_boost = recency * year_norm[idx]
            mmr = base * (1.0 - diversity) + recency_boost - div_penalty
            if mmr > best_score:
                best_score = mmr
                best_idx_in_remaining = pos
        chosen = remaining.pop(best_idx_in_remaining)
        selected.append(chosen)
        chosen_genres = candidates[chosen].get('genres') or []
        if isinstance(chosen_genres, str):
            chosen_genres = chosen_genres.split('|')
        selected_genres.update(chosen_genres)
